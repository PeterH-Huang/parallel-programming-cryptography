...
long long rsa_modExp(long long b, long long e, long long m)
{
    ...
    if (e == 1) return b;
    if (e % 2 == 0) {
        return_value = (rsa_modExp(b * b % m, e / 2, m) % m);
    }
    if (e % 2 == 1) {
        return_value = (b * rsa_modExp(b, (e - 1), m) % m);
    }
...
{
    FILE* primes_list;
    if (fopen_s(&primes_list, PRIME_SOURCE_FILE, "r") != 0)
    {
    ...
    do {
        ...
        int a = static_cast<int>(rand() * (prime_count + 1) / (RAND_MAX + 1.0));

        int b = static_cast<int>(rand() * (prime_count + 1) / (RAND_MAX + 1.0));
